...
class NetStatsPlotter:
    ...
    def plot_net_stats(self, net_data_file):
        ...
        plt.plot(x_stats, y_stats["cwnd"][0], 'k', linewidth=1)
        plt.grid()
        plt.ylim(ymin=0, ymax=400)
        plt.xlim(xmin=0, xmax=x_stats[-1])
        plt.xlabel(y_stats["cwnd"][1]["x"])
        plt.ylabel(y_stats["cwnd"][1]["y"])
        plt.title(y_stats["cwnd"][1]["title"])
        plt.savefig("{}/{}.{}".format(self.save_folder, "cwnd", self.plot_format))
        plt.clf()

        plt.plot(x_stats, y_stats["throughput"][0], 'k', linewidth=1)
        plt.grid()
        plt.xlim(xmin=0, xmax=x_stats[-1])
        plt.ylim(ymin = 0, ymax = 100)
        plt.xlabel(y_stats["throughput"][1]["x"])
        plt.ylabel(y_stats["throughput"][1]["y"])
        plt.title(y_stats["throughput"][1]["title"])
        plt.savefig("{}/{}.{}".format(self.save_folder, "throughput", self.plot_format))
        plt.clf()
